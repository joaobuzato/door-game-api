...
conditionsRouter.get("/conditions", async (request, response) => {
  try {
    ...
    if (e instanceof Error) {
      log(e.message);
      response.status(400).json({ message: "Erro ao obter conditions" }).send();
    }
  }
});

conditionsRouter.get("/conditions/:id", async (request, response) => {
  try {
...
conditionsRouter.post("/conditions", async (request, response) => {
  try {
...
conditionsRouter.put("/conditions/:id", async (request, response) => {
  try {
    const { id } = request.params;
...
conditionsRouter.delete("/conditions/:id", async (request, response) => {
  try {
    const { id } = request.params;
